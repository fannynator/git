...
export class Cat3D {
    ...
    _setupInteraction() {
        ...
        canvas.addEventListener('mousedown', (e) => {
            ...
            const rect = canvas.getBoundingClientRect();
            this._dragStartX = ((e.clientX - rect.left) / rect.width) * 2 - 1;
            this._dragStartY = -((e.clientY - rect.top) / rect.height) * 2 + 1;
            this._dragStartRotX = this._currentRotX;
            this._dragStartRotY = this._currentRotY;
        });
        ...
        canvas.addEventListener('touchstart', (e) => {
            if (e.touches.length === 1) {
                this._isDragging = true;
                const rect = canvas.getBoundingClientRect();
                this._dragStartX = ((e.touches[0].clientX - rect.left) / rect.width) * 2 - 1;
                this._dragStartY = -((e.touches[0].clientY - rect.top) / rect.height) * 2 + 1;
                this._dragStartRotX = this._currentRotX;
                this._dragStartRotY = this._currentRotY;
            }
        }, { passive: true });
        ...
        canvas.addEventListener('touchmove', (e) => {
            if (this._isDragging && e.touches.length === 1) {
                const rect = canvas.getBoundingClientRect();
                const mx = ((e.touches[0].clientX - rect.left) / rect.width) * 2 - 1;
                const my = -((e.touches[0].clientY - rect.top) / rect.height) * 2 + 1;
                this._targetRotX = this._dragStartRotX + (my - this._dragStartY) * 0.8;
                this._targetRotY = this._dragStartRotY + (mx - this._dragStartX) * 1.2;
            }
        }, { passive: true });
    ...
    _updateAnimations(dt) {
        ...
        // Полный обзор 360° — без ограничений
