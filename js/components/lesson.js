...
// Массив для хранения истории ответов: 'correct', 'wrong', 'pending'
let stepHistory = [];
...
async function renderLessonStep() {
    ...
    // Записываем результат в историю (только для не-бонусных заданий)
    if (!task.isBonus && state.lessonStep < stepHistory.length) {
